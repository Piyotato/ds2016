...
public class EACO implements AlgorithmBase {
     ...
     */
    public EACO(double _alpha, int _TTL, int _tabuSize, int _source, int _destination) {
        source = _source;
    ...
    public void addNode(int speed) {
        nodes.add(new Node_EACO(speed, nodes, edgeList, adjMat, alpha, tabuSize));
        cnt.add(0);
    }
    ...
    private void generatePackets() {
        ...
        for (Node_EACO node: nodes) {
            while (cnt.get(node.nodeID) < MAX_ANTS) { // 1 Ant per tick
